...
class FC(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    """
    # TODO
    grad_inputs = np.dot(grads, self.weights)
    return grad_inputs
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    """
    # TODO
    expscores = np.exp(x - np.max(x, axis=1, keepdims=True))
    sumexp = np.sum(expscores, axis=1, keepdims=True)
    probs = expscores / sumexp
    logprobs = np.log(probs)
    loss = -np.mean(np.sum(y * logprobs, axis=1))
    return loss
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    # TODO
    loss = 0.5 * self.weight_decay * np.sum(self.weights ** 2)
    return loss
